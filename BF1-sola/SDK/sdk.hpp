...
public:
	T At(INT nIndex)
	{
		if (!is_ptr_valid(m_firstElement))
			return (T)NULL;

		return *(T*)((uint64_t)m_firstElement + (nIndex * sizeof(T)));
	};
...
public:
	...
	{
		if (!is_ptr_valid(this->m_ptr))
			return NULL;

		if (!is_ptr_valid(&this->m_ptr->m_realptr))
			return NULL;
...
{
public:
	uint64_t m_encryptedPtr;
	uint64_t m_pointerKey;

public:
	T* GetPtr()
	{
		return (T*)(DecryptPointer(this->m_encryptedPtr, (uint64_t)(this->m_pointerKey)));
	}
};
...
public:
	...
	Vec3 GetVehiclePosition()
	{
		...
		Vec3 minSP, maxSP, cor2SP, cor3SP, cor4SP, cor5SP, cor6SP, cor7SP;
		Matrix4x4 _Transform;
		GetTransform(&_Transform);
		Vec3 Position = Vec3(_Transform.m[3][0], _Transform.m[3][1], _Transform.m[3][2]);
		...
		min = glmPos + Mat3Vec3(TransformMatrix, min);
		max = glmPos + Mat3Vec3(TransformMatrix, max);
...
public:
	...
	Vec3 location; //0x0990
	char pad_099C[1712]; //0x099C

	float Distance(Vec3 origin);
	...
	AxisAlignedBox GetAABB() {
		...
		{
			aabb.min = Vec4(-0.350000f, 0.000000f, -0.350000f, 0);
			aabb.max = Vec4(0.350000f, 1.700000f, 0.350000f, 0);
		}
		if (this->poseType == 1) // crouching
		{
			aabb.min = Vec4(-0.350000f, 0.000000f, -0.350000f, 0);
			aabb.max = Vec4(0.350000f, 1.150000f, 0.350000f, 0);
		}
		if (this->poseType == 2) // prone
		{
			aabb.min = Vec4(-0.350000f, 0.000000f, -0.350000f, 0);
			aabb.max = Vec4(0.350000f, 0.400000f, 0.350000f, 0);
		}
		return aabb;
	}
...
public:
	...
	IDXGISwapChain* m_swapChain; //0x0010
